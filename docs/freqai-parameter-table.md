...
| `use_purged_kfold_cv` | Enable Lopez de Prado Purged K-Fold CV for leakage-safe validation inside the training window. When enabled, FreqAI preserves chronological training order even if `shuffle_after_split` is configured. See [Lopez de Prado Methods](freqai-lopez-de-prado.md). <br> **Datatype:** Boolean. <br> Default: `False`.
| `purged_cv_n_splits` | Number of folds used by Purged K-Fold CV. The Lopez de Prado ensemble models train one model per fold. <br> **Datatype:** Positive integer. <br> Default: `5`.
| `purged_cv_embargo_pct` | Fraction of the training data to embargo after each validation fold to reduce temporal leakage. Typical values are `0.01` to `0.05`. <br> **Datatype:** Positive float. <br> Default: `0.01`.
| `purged_cv_enable_purging` | Remove training samples whose label/event interval overlaps the validation fold. <br> **Datatype:** Boolean. <br> Default: `True`.
| `label_horizon_candles` | Forward label/event horizon used when event end timestamps are not provided explicitly. Explicit `train_event_end_times`, `event_end_times`, `train_t1`, or `t1` values are preferred when available. <br> **Datatype:** Positive integer. <br> Default: `10`.
| `ldp_time_decay` | Apply Lopez de Prado-style time-decay sample weights so newer samples can receive more weight. Set to `0` to disable. <br> **Datatype:** Positive float. <br> Default: `0`.
| `ldp_sample_uniqueness` | Weight samples by uniqueness to reduce the impact of overlapping label intervals. Requires chronological timestamps and a label horizon or explicit event end times. <br> **Datatype:** Boolean. <br> Default: `False`.
| `ldp_return_weighting` | Weight samples by return volatility. <br> **Datatype:** Boolean. <br> Default: `False`.
| `ldp_return_weight_span` | EMA span used for return-volatility weighting. <br> **Datatype:** Positive integer. <br> Default: `60`.
| `ldp_fractional_differentiation` | Apply fractional differentiation during FreqAI data preparation to improve stationarity while preserving memory. <br> **Datatype:** Boolean. <br> Default: `False`.
| `ldp_frac_diff_order` | Fractional differentiation order `d`. Lower values preserve more memory. <br> **Datatype:** Positive float. <br> Default: `0.5`.
| `ldp_frac_diff_threshold` | Weight cutoff threshold for fractional differentiation. <br> **Datatype:** Positive float. <br> Default: `0.01`.
| `ldp_frac_diff_columns` | Optional list of columns to fractionally differentiate. If omitted, all eligible feature columns are considered. <br> **Datatype:** List of strings or `null`. <br> Default: `null`.
